    ...
    if (count($storyarray) > 0) {
        ...
        foreach ($storyarray as $newstory) {
            ...
            echo "</td><td align='center' class='nw'>"
                 . formatTimestamp($newstory->created())
        ...
        foreach ($storyarray as $autostory) {
            ...
            echo "
        		<tr><td align='center'><b>"
                 ...
                 . "</td><td align='center'>"
                 . $expire
                 . "</td><td align='center'><a href='index.php?op=edit&amp;storyid="
                 . $autostory->storyid()
                 . "'>"
                 . _AM_EDIT
...
function lastStories()
{
    ...
    $storyarray = NewsStory::getPublished(10, 0, 0, '', 1);
    ...
    foreach ($storyarray as $eachstory) {
		...
		<td align='center'><b>"
             ...
             . "'>"
             . _AM_DELETE
             . "</a>-<a href='index.php?op=moveup&amp;storyid="
             . $eachstory->storyid()
             . "'>"
             . _AM_MOVEUP
             . '</a>';
    ...
    echo "<form action='index.php' method='post'>
    	...
    	<option value='moveup'>" . _AM_MOVEUP . "</option>
    	</select>
...
function topicsmanager()
{
    ...
    $topics_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH . '/modules/' . $xoopsModule->dirname() . '/images/topics/');
...
switch ($op) {
    case 'moveup':
        ...
        redirect_header(XOOPS_URL, 1, _AM_DBUPDATED);
        exit();
        break;
    case 'moveitup':
        if (isset($storyid)) {
    ...
    case 'edit':
        ...
        $storyurl = $story->storyurl('Edit');
        $author = $story->author('Edit');
    ...
    case 'preview':
        ...
        if (isset($nohtml) && (0 == $nohtml || 1 == $nohtml)) {
        ...
        $p_storyurl = $story->storyurl('Preview');
        $p_author = $story->author('Preview');
        $p_hometext = $story->hometext('Preview');
        $p_bodytext = $story->bodytext('Preview');
        $title = $story->title('InForm');
        $storyurl = $story->storyurl('InForm');
        $author = $story->author('InForm');
        ...
        if ($topicdisplay) {
            ...
            if (empty($topicid)) {
                ...
                if ('' != $xt->topic_imgurl() && file_exists(XOOPS_ROOT_PATH . '/modules/".$xoopsModule->dirname()."/images/topics/' . $xt->topic_imgurl())) {
                    $timage = "<img src='" . XOOPS_URL . '/modules/' . $xoopsModule->dirname() . '/images/topics/' . $xt->topic_imgurl() . "' align='$align' border='0' hspace='10' vspace=10'>";
                }
            }
        }
    ...
    case 'save':
        ...
        //			$story->setNotifyPub($notifypub);
            ...
            if (!empty($autodate)) {
                ...
                $offset -= $xoopsConfig['server_TZ'];

                $pubdate -= ($offset * 3600);

                $story->setPublished($pubdate);
            } elseif ((0 == $story->published()) && $approve) {
                $story->setPublished(time());
